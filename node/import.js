...
async function insertProducts() {
  try {
    ...
    const insertQuery = 'INSERT INTO products (name, price, image, brand, category, stock) VALUES (?, ?, ?, ?, ?, ?)';

    for (const product of products) {
      const { name, price, image, brand, category, stock } = product;

      try {
        await connection.execute(insertQuery, [name, price, image, brand, category, stock]);
